...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    fig.suptitle("Rata-rata Purchase Amount vs Variabel Kategorikal",
                 fontsize=13, fontweight="bold")
    for ax, (col, title) in zip(axes, cat_targets):
...
→ Pria berlangganan = segmen paling aktif pakai promo, namun tidak meningkatkan nilai transaksi.""")
    ...
    with c1:
        ...
        ax.set_title("Rate per Gender", fontweight="bold")
        ax.set_ylim(0, gen_pct.max() * 1.25)
    ...
    with c2:
        ...
        fig, ax = base_fig(4, 3.5)
        bars = ax.bar(sub_pct.index, sub_pct.values, color=[PAL[2], PAL[5]],
                      edgecolor="white", width=0.5)
        ax.set_title("Rate per Subscription", fontweight="bold")
        ax.set_ylim(0, sub_pct.max() * 1.25)
        ax.set_ylabel("% dari total grup tsb")
        ax.spines[["top","right"]].set_visible(False)
    ...
    insight("""Rate Free Shipping antara semua segmen (gender, subscription, frekuensi) sangat serupa (~17%) → <b>tidak ada tipe pelanggan yang secara khusus mendominasi pilihan Free Shipping</b>. Konsisten dengan pola distribusi uniform dataset ini.
Free Shipping dapat dijadikan <b>insentif threshold</b> (misal: gratis ongkir untuk pembelian >$75) untuk mendorong peningkatan Average Order Value.""")
...
**🎫 Efektivitas Promo & 🏬 Store Pickup**
- Promo: No = \$60.1 vs Yes = \$59.3 → tidak meningkatkan nilai transaksi
- Store Pickup: adopsi rata-rata ~17%, variasi besar antar state namun minim antar musim & kategori
...
- Pertimbangkan threshold Store Pickup sebagai insentif lokal
        """)
    with col3:
        st.markdown(r"""
**📦 C. Produk & Musim**
